...
router.get('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const result = await pool.query(`SELECT u.id, u.login, u.role_id, r.name AS role_name, u.unit_id,
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const userResult = await client.query(`INSERT INTO users (login, password_hash, role_id, unit_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, login, role_id, unit_id`, [login, password_hash, role_id, unit_id || null]);
        const user = userResult.rows[0];
        if (hasFio && unit_id) {
            await client.query(`INSERT INTO employees (user_id, last_name, first_name, middle_name, unit_id)
         VALUES ($1, $2, $3, $4, $5)`, [user.id, last_name.trim(), first_name.trim(), middle_name?.trim() || null, unit_id]);
        }
...
router.put('/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        if (login) {
            const loginCheck = await client.query('SELECT id FROM users WHERE login = $1 AND id != $2', [login, id]);
        ...
        if (unit_id !== undefined) {
            fields.push(`unit_id = $${paramIdx++}`);
            values.push(unit_id || null);
        }
        let userRow: Record<string, unknown> = existing.rows[0];
        if (fields.length > 0) {
            values.push(id);
            const result = await client.query(`UPDATE users SET ${fields.join(', ')} WHERE id = $${paramIdx}
         RETURNING id, login, role_id, unit_id`, values);
            userRow = result.rows[0];
        }
        if (last_name !== undefined || first_name !== undefined || middle_name !== undefined) {
            const empCheck = await client.query('SELECT id FROM employees WHERE user_id = $1 LIMIT 1', [id]);
            if (empCheck.rows.length > 0) {
                const empFields: string[] = [];
                const empValues: unknown[] = [];
                let ei = 1;
                if (last_name !== undefined) {
                    empFields.push(`last_name = $${ei++}`);
                    empValues.push(last_name?.trim() || null);
                }
                if (first_name !== undefined) {
                    empFields.push(`first_name = $${ei++}`);
                    empValues.push(first_name?.trim() || null);
                }
                if (middle_name !== undefined) {
                    empFields.push(`middle_name = $${ei++}`);
                    empValues.push(middle_name?.trim() || null);
                }
                if (empFields.length > 0) {
                    empValues.push(id);
                    await client.query(`UPDATE employees SET ${empFields.join(', ')} WHERE user_id = $${ei}`, empValues);
                }
            }
                ...
                if (unitForEmp) {
                    await client.query(`INSERT INTO employees (user_id, last_name, first_name, middle_name, unit_id)
             VALUES ($1, $2, $3, $4, $5)`, [id, last_name.trim(), first_name.trim(), middle_name?.trim() || null, unitForEmp]);
                }
            }
        }
        await client.query('COMMIT');
...
router.delete('/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const raskhodCheck = await client.query('SELECT id FROM raskhod WHERE created_by_user_id = $1 LIMIT 1', [id]);
        if (raskhodCheck.rows.length > 0) {
            res.status(409).json({ error: 'Нельзя удалить: у пользователя есть записи расхода личного состава.' });
            return;
        }
        const alertsCheck = await client.query('SELECT id FROM alerts WHERE created_by_user_id = $1 LIMIT 1', [id]);
