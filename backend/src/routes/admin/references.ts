// CRUD для справочников: positions, ranks, units, user_statuses (только admin)
import { Router, Request, Response } from 'express';
import { pool } from '../../db';

// Зависимые таблицы для проверки при удалении
const DEPENDENCIES: Record<string, Array<{ table: string; column: string }>> = {
...
/**
 * Создаёт роутер CRUD для указанного справочника.
...
function createReferenceRouter(tableName: string): Router {
  ...
  router.get('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      ...
      const result = await pool.query(
        `SELECT id, name FROM ${tableName} ORDER BY id LIMIT $1 OFFSET $2`,
        [page_size, offset]
      );
  ...
  // POST /api/{resource} — создание записи { name }
  ...
  // PUT /api/{resource}/:id — обновление { name }
  ...
  // DELETE /api/{resource}/:id — удаление с проверкой зависимостей
  router.delete('/:id', async (req: Request, res: Response): Promise<void> => {
    const { id } = req.params;

    try {
      // Проверяем наличие ссылок в зависимых таблицах
...
// Экспортируем отдельные роутеры для каждого справочника
