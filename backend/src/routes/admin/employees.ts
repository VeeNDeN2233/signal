...
router.get('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const countResult = await pool.query(`SELECT COUNT(*) FROM employees e ${unit_id ? 'WHERE e.unit_id = $1' : ''}`, countParams);
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
    const { user_id, last_name, first_name, middle_name, position_id, rank_id, birth_date, unit_id, phone_number, } = req.body;
    ...
    try {
       ...
       RETURNING id, user_id, last_name, first_name, middle_name,
                 position_id, rank_id, birth_date, unit_id, phone_number`, [user_id ?? null, last_name, first_name, middle_name ?? null,
...
router.put('/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        values.push(id);
        const result = await pool.query(`UPDATE employees SET ${fields.join(', ')} WHERE id = $${values.length}
       RETURNING id, user_id, last_name, first_name, middle_name,
                 position_id, rank_id, birth_date, unit_id, phone_number`, values);
        res.json({ data: result.rows[0] });
    }
    catch (err) {
