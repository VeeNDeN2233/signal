...
function formatInitialsLastName(lastName: string, firstName: string, middleName: string | null | undefined): string {
...
router.post('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        ...
        const dupCheck = await client.query('SELECT id FROM raskhod WHERE raskhod_date = $1 AND raskhod_time = $2 AND unit_id = $3', [raskhod_date, raskhod_time, unit_id]);
        if (dupCheck.rows.length > 0) {
            await client.query('ROLLBACK');
            res.status(409).json({ error: 'Расход на данную дату и время уже существует' });
            return;
        }
        const raskhodResult = await client.query(`INSERT INTO raskhod (raskhod_date, raskhod_time, created_by_user_id, unit_id)
       VALUES ($1, $2, $3, $4)
       RETURNING id, raskhod_date, raskhod_time, unit_id`, [raskhod_date, raskhod_time, created_by_user_id, unit_id]);
        const raskhod = raskhodResult.rows[0];
        for (const entry of entries) {
            await client.query('INSERT INTO raskhod_entries (raskhod_id, employee_id, status_id) VALUES ($1, $2, $3)', [raskhod.id, entry.employee_id, entry.status_id]);
        }
...
router.get('/', async (req: Request, res: Response): Promise<void> => {
    ...
    try {
        const countResult = await pool.query('SELECT COUNT(*) FROM raskhod WHERE unit_id = $1', [unit_id]);
...
router.get('/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    const unit_id = req.user!.unit_id;
    try {
        const raskhodResult = await pool.query(`SELECT r.id, r.raskhod_date, r.raskhod_time, r.unit_id, r.created_by_user_id, u.name AS unit_name
       FROM raskhod r
       ...
       WHERE r.id = $1`, [id]);
        ...
        const entriesResult = await pool.query(`SELECT
         re.employee_id,
...
router.put('/:id', async (req: Request, res: Response): Promise<void> => {
    ...
    const client = await pool.connect();
    try {
        const raskhodResult = await client.query('SELECT id, unit_id FROM raskhod WHERE id = $1', [id]);
        if (raskhodResult.rows.length === 0) {
            res.status(404).json({ error: 'Расход не найден' });
            return;
        }
        ...
        for (const entry of entries) {
            await client.query('INSERT INTO raskhod_entries (raskhod_id, employee_id, status_id) VALUES ($1, $2, $3)', [id, entry.employee_id, entry.status_id]);
        }
        await client.query('COMMIT');
...
router.get('/:id/download', async (req: Request, res: Response): Promise<void> => {
    ...
    const unit_id = req.user!.unit_id;
    try {
        const raskhodResult = await pool.query(`SELECT r.id, r.raskhod_date, r.raskhod_time, r.unit_id, u.name AS unit_name
       ...
       WHERE r.id = $1`, [id]);
        ...
        const entriesResult = await pool.query(`SELECT e.last_name, e.first_name, e.middle_name, us.name AS status_name
       FROM raskhod_entries re
       JOIN employees e ON e.id = re.employee_id
       JOIN user_statuses us ON us.id = re.status_id
       WHERE re.raskhod_id = $1
       ORDER BY e.last_name, e.first_name`, [id]);
        ...
        const chiefResult = await pool.query(`SELECT e.last_name, e.first_name, e.middle_name, rk.name AS rank_name
       FROM employees e
       JOIN positions p ON p.id = e.position_id
       LEFT JOIN ranks rk ON rk.id = e.rank_id
       WHERE e.unit_id = $1
         AND (p.name ILIKE '%начальник%курс%' OR p.name = 'Начальник курса')
       LIMIT 1`, [raskhod.unit_id]);
        ...
        const summaryParagraphs = [
            ...
            ...statusKeysSorted.map((statusName) => new Paragraph({
                children: [
                    new TextRun({
