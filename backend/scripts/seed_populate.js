...
function transliterate(str) {
  ...
  return str.toLowerCase().split('').map(c => {
    ...
    if (c.charCodeAt(0) === 0x44a || c.charCodeAt(0) === 0x44c) return ''; // ъ, ь
...
async function run() {
  ...
  try {
    ...
    function makeLogin(lastName) {
      ...
      while (usedLogins.has(login)) login += String(Math.floor(Math.random() * 100));
    ...
    async function createPerson(roleId, positionId, rankId, unitId) {
      ...
      const userRes = await client.query(
        'INSERT INTO users (login, password_hash, role_id, unit_id) VALUES ($1, $2, $3, $4) RETURNING id',
        [login, hash, roleId, unitId]
      );
      await client.query(
        `INSERT INTO employees (user_id, last_name, first_name, middle_name, position_id, rank_id, unit_id, phone_number)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`,
        [userRes.rows[0].id, last, first, middle, positionId, rankId, unitId, genPhone()]
      );
