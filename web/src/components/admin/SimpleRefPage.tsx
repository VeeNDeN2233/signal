...
export function SimpleRefPage({ title, endpoint }: SimpleRefPageProps) {
    const { rows, meta, page, loading, error, fetchPage, create, update, remove } = useCrud<NamedItem>(endpoint);
    ...
    return (<div>
      ...
      {loading ? (<p style={{ color: '#64748b' }}>Загрузка...</p>) : (<CrudTable<NamedItem> columns={[
                { key: 'id', label: 'ID' },
      ...
      {modalOpen && (<Modal title={editing ? `Редактировать: ${editing.name}` : `Добавить — ${title}`} onClose={() => setModalOpen(false)}>
