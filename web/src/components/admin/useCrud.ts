...
export function useCrud<T extends {
    id: number;
}>(endpoint: string, pageSize = 20) {
    ...
    const create = useCallback(async (body: Partial<T>): Promise<string | null> => {
        try {
    ...
    const update = useCallback(async (id: number, body: Partial<T>): Promise<string | null> => {
        try {
...
function extractError(e: unknown): string {
    if (axios.isAxiosError(e)) {
        ...
        if (e.response?.status === 409)
            return 'Конфликт: запись уже существует или используется.';
