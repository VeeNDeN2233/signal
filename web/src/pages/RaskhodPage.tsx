...
export function RaskhodPage() {
    ...
    function setEntryStatus(employeeId: number, statusId: number) {
        setEntries((prev) => prev.map((e) => (e.employee_id === employeeId ? { ...e, status_id: statusId } : e)));
    }
    ...
    return (<>
      ...
      {loading ? (<p style={{ color: '#64748b' }}>Загрузка...</p>) : (<form onSubmit={handleSubmit}>
          
          <div style={controlsRowStyle}>
            <div style={fieldGroupStyle}>
              <label style={labelStyle}>Дата</label>
              <input type="date" value={date} onChange={(e) => { setDate(e.target.value); setSubmitSuccess(false); setSubmitError(null); }} style={inputStyle}/>
            </div>
            <div style={fieldGroupStyle}>
              <label style={labelStyle}>Время</label>
              <div style={{ display: 'flex', gap: 16, alignItems: 'center', paddingTop: 6 }}>
                {(['09:00', '21:00'] as const).map((t) => (<label key={t} style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer', fontSize: 14 }}>
                    <input type="radio" name="raskhod_time" value={t} checked={time === t} onChange={() => { setTime(t); setSubmitSuccess(false); setSubmitError(null); }}/>
                    {t}
                  </label>))}
              </div>
            </div>
          ...
          {employees.length === 0 ? (<p style={{ color: '#64748b' }}>Нет сотрудников в подразделении</p>) : (<>
              
              <div style={{ marginBottom: 12 }}>
                <input type="text" placeholder="Поиск по ФИО..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} style={{ ...inputStyle, width: '100%', maxWidth: 400 }}/>
              </div>

              {filteredEmployees.length === 0 ? (<p style={{ color: '#64748b' }}>Сотрудники не найдены</p>) : (<div style={tableWrapStyle}>
                  <table style={tableStyle}>
                    ...
                    <tbody>
                      {filteredEmployees.map((emp, idx) => {
                        ...
                        return (<tr key={emp.id} style={idx % 2 === 0 ? rowEvenStyle : rowOddStyle}>
                            ...
                            <td style={{ ...tdStyle, paddingTop: 6, paddingBottom: 6 }}>
                              <div style={statusChipsRow}>
                                ...
                                return (<button key={s.id} type="button" onClick={() => setEntryStatus(emp.id, s.id)} style={active
          ...
          <div style={{ marginTop: 16 }}>
            <button type="submit" disabled={submitting || employees.length === 0} style={submitting || employees.length === 0 ? disabledBtnStyle : submitBtnStyle}>
