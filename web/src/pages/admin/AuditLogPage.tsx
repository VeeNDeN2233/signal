    ...
    return (<div>
      ...
      <div style={toolbarStyle}>
        <div style={filterGroupStyle}>
          {FILTER_OPTIONS.map((opt) => (<button key={opt.value} type="button" onClick={() => handleFilterChange(opt.value)} style={filter === opt.value ? filterBtnActiveStyle : filterBtnStyle}>
              {opt.label}
            </button>))}
        </div>
        <input type="search" value={searchInput} onChange={(e) => setSearchInput(e.target.value)} placeholder="Поиск: логин, подразделение, тип…" style={searchInputStyle} aria-label="Поиск по журналу"/>
      ...
      {loading ? (<p style={{ color: '#64748b' }}>Загрузка...</p>) : entries.length === 0 ? (<p style={{ color: '#64748b' }}>Событий не найдено</p>) : (<>
          <div style={tableWrapStyle}>
            <table style={tableStyle}>
              ...
              <tbody>
                {entries.map((e, idx) => (<tr key={`${e.event_type}-${e.event_id}`} style={idx % 2 === 0 ? rowEvenStyle : rowOddStyle}>
          ...
          {totalPages > 1 && (<div style={paginationStyle}>
              <button type="button" onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1} style={page === 1 ? disabledBtnStyle : pageBtnStyle}>
                Назад
              </button>
              <span style={{ fontSize: 14, color: '#374151' }}>Стр. {page} / {totalPages}</span>
              <button type="button" onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page === totalPages} style={page === totalPages ? disabledBtnStyle : pageBtnStyle}>
                Вперёд
              </button>
            </div>)}
        </>)}
