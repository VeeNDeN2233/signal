...
export function RosterPage() {
  ...
  const [freeUsers, setFreeUsers] = useState<{ id: number; login: string }[]>([])
  ...
  useEffect(() => {
    ...
    apiClient.get<{ data: Unit[] }>('/units', { params: p }).then((r) => setUnits(r.data.data)).catch(() => {})
    apiClient.get<{ data: NamedItem[] }>('/ranks', { params: p }).then((r) => setRanks(r.data.data)).catch(() => {})
    apiClient.get<{ data: NamedItem[] }>('/positions', { params: p }).then((r) => setPositions(r.data.data)).catch(() => {})
    apiClient.get<{ data: Role[] }>('/roles').then((r) => {
  ...
  const fetchEmployees = useCallback((unitId: number) => {
    setLoadingEmps(true)
    setLoadError(null)
    apiClient.get<{ data: Employee[] }>('/employees', { params: { unit_id: unitId, page: 1, page_size: 200 } })
  ...
  // Пользователи без привязанного сотрудника
  async function loadFreeUsers(currentEmpId?: number) {
    try {
      const r = await apiClient.get<{ data: { id: number; login: string; last_name: string | null }[] }>(
        '/users', { params: { page: 1, page_size: 200 } }
      )
      const linkedIds = new Set(
        employees.filter((e) => e.user_id && e.id !== currentEmpId).map((e) => e.user_id)
      )
  ...
  async function handleDelete(emp: Employee) {
    if (!confirm(`Удалить сотрудника «${emp.last_name} ${emp.first_name}»?`)) return
    try {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (editing) {
        ...
        if (editing.user_id) {
          // Уже есть аккаунт — обновляем пароль если введён
          if (form.password.trim()) {
            await apiClient.put(`/users/${editing.user_id}`, { password: form.password })
          }
        } else {
          // Нет аккаунта — создаём или привязываем
          if (form.account_mode === 'new') {
            ...
            // Синхронизируем unit_id пользователя
      ...
      setModalOpen(false)
      if (selectedUnitId) fetchEmployees(selectedUnitId)
    } catch (err: unknown) {
  ...
  return (
    <div>
        ...
        <div style={unitButtonsRow}>
          {units.map((u) => (
            <button
              ...
              style={selectedUnitId === u.id ? unitBtnActive : unitBtnInactive}
            >
              {u.name}
            </button>
          ))}
            ...
            <table style={tableStyle}>
              ...
              <tbody>
                {filteredEmployees.map((emp, idx) => (
                  <tr key={emp.id} style={idx % 2 === 0 ? rowEven : rowOdd}>
                    ...
                    <td style={tdCenter}>
                      {emp.user_login ? (
                        <span style={accountBadge}>{emp.user_login}</span>
                      ) : (
                        <span style={noAccountBadge}>нет</span>
                      )}
                    </td>
                    <td style={tdCenter}>
      ...
      {modalOpen && (
        <Modal
          title={editing ? `${editing.last_name} ${editing.first_name}` : 'Новый сотрудник'}
        ...
        >
          <form onSubmit={handleSubmit}>
            ...
            {editing?.user_login ? (
              // ── Уже есть аккаунт — показываем логин и смену пароля
              <>
              ...
              // ── Аккаунта нет — предлагаем создать или привязать
              <>
                ...
                {form.account_mode === 'link' && (
                  <Field label="Существующий пользователь">
