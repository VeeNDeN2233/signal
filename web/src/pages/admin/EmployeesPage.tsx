...
export function EmployeesPage() {
    const { rows, meta, page, loading, error, fetchPage, create, update, remove } = useCrud<Employee>('/employees');
    ...
    useEffect(() => {
        ...
        apiClient.get<{
            data: NamedItem[];
        }>('/positions', { params }).then((r) => setPositions(r.data.data)).catch(() => { });
        apiClient.get<{
            data: NamedItem[];
        }>('/ranks', { params }).then((r) => setRanks(r.data.data)).catch(() => { });
        apiClient.get<{
            data: NamedItem[];
        }>('/units', { params }).then((r) => setUnits(r.data.data)).catch(() => { });
        apiClient.get<{
            data: UserItem[];
        }>('/users', { params }).then((r) => setUsers(r.data.data)).catch(() => { });
    }, []);
    ...
    async function handleDelete(row: Employee) {
        if (!confirm(`Удалить сотрудника «${row.last_name} ${row.first_name}»?`))
            return;
    ...
    async function handleSubmit(e: React.FormEvent) {
        ...
        if (!form.unit_id) {
            setFormError('Подразделение обязательно');
            return;
        }
        ...
        try {
            ...
            if (!editing && form.user_type) {
                const roleId = form.user_type === 'commander' ? 3 : 1;
    ...
    return (<div>
      ...
      {loading ? (<p style={{ color: '#64748b' }}>Загрузка...</p>) : (<CrudTable<Employee> columns={[
                { key: 'id', label: 'ID' },
            ...
            ]} rows={rows} page={page} pageSize={meta.page_size} total={meta.total} onPageChange={fetchPage} onAdd={openAdd} onEdit={openEdit} onDelete={handleDelete}/>)}

      {modalOpen && (<Modal title={editing ? `Редактировать сотрудника` : 'Добавить сотрудника'} onClose={() => setModalOpen(false)}>
