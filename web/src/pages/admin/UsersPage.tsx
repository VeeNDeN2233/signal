...
export function UsersPage() {
    const { rows, meta, page, loading, error, fetchPage, create, update, remove } = useCrud<User>('/users');
    ...
    useEffect(() => {
        apiClient.get<{
            data: Role[];
        }>('/roles').then((r) => setRoles(r.data.data)).catch(() => {
            setRoles([
                { id: 1, name: 'user' },
                { id: 2, name: 'admin' },
                { id: 3, name: 'commander' },
        ...
        apiClient.get<{
            data: Unit[];
        }>('/units', { params: { page: 1, page_size: 100 } })
    ...
    async function handleDelete(row: User) {
        if (!confirm(`Удалить пользователя «${row.login}»?`))
            return;
    ...
    return (<div>
      ...
      {loading ? (<p style={{ color: '#64748b' }}>Загрузка...</p>) : (<CrudTable<User> columns={[
                { key: 'id', label: 'ID' },
      ...
      {modalOpen && (<Modal title={editing ? `Редактировать: ${editing.login}` : 'Добавить пользователя'} onClose={() => setModalOpen(false)}>
