...
function groupByMonth(records: RaskhodRecord[]): {
    monthKey: string;
    items: RaskhodRecord[];
}[] {
    const map = new Map<string, RaskhodRecord[]>();
    for (const r of records) {
        const key = r.raskhod_date.slice(0, 7);
        if (!map.has(key))
            map.set(key, []);
        map.get(key)!.push(r);
    }
...
export function RaskhodHistoryPage() {
    ...
    useEffect(() => {
        apiClient.get<{
            data: UserStatus[];
        }>('/employees/statuses').then((r) => setStatuses(r.data.data)).catch(() => { });
    }, []);
    const fetchAll = useCallback(() => {
        ...
        apiClient.get<{
            data: RaskhodRecord[];
            meta: Meta;
        }>('/raskhod', { params: { page: 1, page_size: 200 } })
    ...
    const filtered = allRecords.filter((r) => {
        if (filterTime !== 'all' && r.raskhod_time.slice(0, 5) !== filterTime)
            return false;
        if (searchDate.trim()) {
            const q = searchDate.trim().toLowerCase();
            const formatted = formatDate(r.raskhod_date);
            const iso = r.raskhod_date;
            if (!formatted.includes(q) && !iso.includes(q))
                return false;
        }
        return true;
    });
    ...
    async function deleteRecord() {
        if (!selected)
            return;
        const ok = confirm(`Удалить расход от ${formatDate(selected.raskhod_date)} ${selected.raskhod_time}? Действие необратимо.`);
        if (!ok)
            return;
        setDeleting(true);
        try {
    ...
    if (detail) {
        ...
        for (const e of detail.entries)
            map.set(e.status_name, (map.get(e.status_name) ?? 0) + 1);
    ...
    return (<>
      ...
      <div style={filtersRowStyle}>
        <div style={filterGroupStyle}>
          ...
          <input type="text" placeholder="например: 15.04 или 2025" value={searchDate} onChange={(e) => setSearchDate(e.target.value)} style={filterInputStyle}/>
        </div>
        <div style={filterGroupStyle}>
          <label style={filterLabelStyle}>Время расхода</label>
          <div style={radioRowStyle}>
            {(['all', '09:00', '21:00'] as const).map((v) => (<label key={v} style={radioLabelStyle}>
        ...
        <div style={filterStatsStyle}>
          ...
          {(searchDate || filterTime !== 'all') && (<button onClick={() => { setSearchDate(''); setFilterTime('all'); }} style={clearBtnStyle}>
              Сбросить
            </button>)}
        </div>
      </div>
      ...
      <div style={layoutStyle}>
        ...
        <div style={listColumnStyle}>
          ...
          {loading ? (<p style={{ color: '#64748b', padding: '12px 0' }}>Загрузка...</p>) : filtered.length === 0 ? (<p style={{ color: '#94a3b8', padding: '12px 0', fontSize: 14 }}>
              {allRecords.length === 0 ? 'Расходов пока нет' : 'Нет записей по фильтру'}
            </p>) : (<div style={listStyle}>
              {grouped.map(({ monthKey, items }) => (<div key={monthKey}>
                  <div style={monthHeaderStyle}>
                    {formatMonthYear(monthKey + '-01')}
                  </div>
                  {items.map((rec) => {
                    const isActive = selected?.id === rec.id;
                    return (<div key={rec.id} onClick={() => selectRecord(rec)} style={isActive ? listItemActiveStyle : listItemStyle}>
        ...
        <div style={detailColumnStyle}>
          {!selected && (<div style={emptyDetailStyle}>
          ...
          {selected && (<div style={detailCardStyle}>
              ...
              <div style={detailTopStyle}>
                ...
                <div style={detailActionsStyle}>
                  {!editMode ? (<>
                      <button onClick={downloadDocx} disabled={!detail || downloading} style={!detail || downloading ? disabledBtnStyle : downloadBtnStyle}>
                        {downloading ? 'Формирование...' : 'Скачать DOCX'}
                      </button>
                      <button onClick={startEdit} disabled={!detail} style={!detail ? disabledBtnStyle : editBtnStyle}>
                        Редактировать
                      </button>
                      <button onClick={deleteRecord} disabled={!detail || deleting} style={!detail || deleting ? disabledBtnStyle : deleteBtnStyle}>
              ...
              {!editMode && statusSummary.length > 0 && (<div style={summaryRowStyle}>
                  <span style={{ fontSize: 12, color: '#94a3b8', marginRight: 8, alignSelf: 'center' }}>Итого:</span>
                  {statusSummary.map(({ name, count }) => (<span key={name} style={summaryChipStyle(name)}>
                      {name} — {count}
                    </span>))}
                </div>)}
              ...
              {detail && (<table style={tableStyle}>
                  ...
                  <tbody>
                    ...
                    return (<tr key={entry.employee_id} style={i % 2 === 0 ? rowEvenStyle : rowOddStyle}>
                          ...
                          <td style={{ ...tdStyle, paddingTop: 6, paddingBottom: 6 }}>
                            {editMode ? (<div style={chipsRowStyle}>
                                {statuses.map((s) => {
                                const active = selectedStatusId === s.id;
                                const isPresent = s.name.toLowerCase() === 'налицо';
                                return (<button key={s.id} type="button" onClick={() => setEditStatus(entry.employee_id, s.id)} style={active ? (isPresent ? chipGreen : chipRed) : chipGray}>
                                      {s.name}
                                    </button>);
                            })}
                              </div>) : (<span style={statusBadge(entry.status_name)}>{entry.status_name}</span>)}
                          </td>
                        </tr>);
                })}
                  </tbody>
...
function timeTagStyle(time: string, active: boolean): React.CSSProperties {
    if (active)
        return { fontSize: 11, fontWeight: 600, color: '#bfdbfe' };
