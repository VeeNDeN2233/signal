...
export function AlertsHistoryPage() {
    ...
    const fetchHistory = useCallback(() => {
        ...
        apiClient.get<{
            data: AlertRecord[];
            meta: Meta;
        }>('/alerts', { params: { page: 1, page_size: 200 } })
    ...
    const filteredRecords = useMemo(() => {
        return records.filter((rec) => {
            if (coverageFilter === 'full' && rec.total_employees !== rec.responded_count)
                return false;
            if (coverageFilter === 'partial' && rec.total_employees === rec.responded_count)
                return false;
            if (!search.trim())
                return true;
    ...
    const filteredDetail = useMemo(() => {
        ...
        return detail.filter((resp) => [resp.last_name, resp.first_name, resp.middle_name].filter(Boolean).join(' ').toLowerCase().includes(q));
    }, [detail, detailSearch]);
    ...
    return (<>
      ...
      <div style={filtersRowStyle}>
        <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Поиск по дате/времени..." style={searchInputStyle}/>
      ...
      <div style={layoutStyle}>
        <div style={leftPaneStyle}>
          ...
          {loading ? (<p style={{ color: '#64748b' }}>Загрузка...</p>) : filteredRecords.length === 0 ? (<p style={{ color: '#64748b' }}>Записей не найдено</p>) : (groupedRecords.map((group) => (<div key={group.key} style={{ marginBottom: 12 }}>
                <div style={monthHeaderStyle}>{formatMonthYear(group.items[0].created_at)}</div>
                {group.items.map((rec) => {
                const isSelected = selectedId === rec.id;
                return (<button key={rec.id} onClick={() => toggleDetail(rec.id)} style={isSelected ? recordBtnActive : recordBtn}>
        ...
        <div style={rightPaneStyle}>
          ...
          {selectedId && (<div style={detailCardStyle}>
              ...
              {detail && (<>
                  ...
                  <input type="text" value={detailSearch} onChange={(e) => setDetailSearch(e.target.value)} placeholder="Поиск сотрудника по ФИО..." style={{ ...searchInputStyle, marginBottom: 10 }}/>
                  <table style={{ ...tableStyle, fontSize: 13 }}>
                    ...
                    <tbody>
                      {filteredDetail.map((resp, i) => (<tr key={resp.employee_id} style={i % 2 === 0 ? rowEvenStyle : rowOddStyle}>
                          ...
                          <td style={{ ...tdCenterStyle, fontSize: 13 }}>
                            {resp.responded_at !== null ? (<span style={statusOk}>Принял</span>) : (<span style={statusNo}>Не ответил</span>)}
                          </td>
