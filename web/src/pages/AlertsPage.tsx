...
export function AlertsPage() {
  ...
  const fetchResponses = async (id: number) => {
    try {
      const res = await apiClient.get<{ data: AlertResponse[] }>(`/alerts/${id}/responses`)
      setResponses(res.data.data)
      setError(null)
    } catch (err: unknown) {
      // Если тревога не найдена (404) — сбрасываем
  ...
  // Запуск polling при наличии активной тревоги
  useEffect(() => {
    if (alertId !== null) {
  ...
  const handleCreateAlert = async () => {
    ...
    setError(null)
    try {
      const res = await apiClient.post<{ data: Alert }>('/alerts')
      ...
      {alertId === null ? (
        <div>
          ...
          <button
            ...
            disabled={creating}
            style={creating ? disabledBtnStyle : alertBtnStyle}
          >
              ...
              <table style={tableStyle}>
                ...
                <tbody>
                  {responses.map((resp, idx) => (
                    <tr key={resp.employee_id} style={idx % 2 === 0 ? rowEvenStyle : rowOddStyle}>
                      <td style={tdCenterStyle}>{idx + 1}</td>
                      <td style={tdStyle}>{fullName(resp)}</td>
                      <td style={tdCenterStyle}>
                        {resp.responded_at !== null ? (
                          <span style={statusBadgeResponded}>Принял</span>
                        ) : (
                          <span style={statusBadgeNotResponded}>Не ответил</span>
                        )}
                      </td>
